...
class WalkEncoder(nn.Module):
    def __init__(
        ...
        if sequence_layer_type == "conv":
            ...
            self.seq_layer = S4Block(d_model=hidden_size, lr=0.001)
            if bidirection:
                self.seq_layer_backward = S4Block(d_model=hidden_size, lr=0.001)
            ...
            raise NotImplementedError(f"not supported sequence layer type: {sequence_layer_type}")
