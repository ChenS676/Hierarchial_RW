...
class MessagePassingLayer(nn.Module):
    def __init__(
        ...
        self.global_mp = _get_global_layer(
            global_model_type, hidden_size, num_heads, attn_dropout, dropout,
            vn_norm_first=vn_norm_first, vn_norm_type=vn_norm_type, vn_pooling=vn_pooling,
        )
...
def _get_global_layer(global_model_type, hidden_size, num_heads, attn_dropout, dropout,
                      vn_norm_first=True, vn_norm_type='batchnorm', vn_pooling='sum'):
    if global_model_type == 'transformer':
        return nn.MultiheadAttention(hidden_size, num_heads, dropout=attn_dropout, batch_first=True)
    elif global_model_type == 'performer':
        from performer_pytorch import SelfAttention
        return SelfAttention(dim=hidden_size, heads=num_heads, dropout=attn_dropout, causal=False)
    elif global_model_type == 'vn':
        return VirtualNodeLayer(
            hidden_size, dropout,
            norm_first=vn_norm_first, norm_type=vn_norm_type, pooling=vn_pooling
        )
    else:
        return None
