...
def build_run_name(config: dict) -> str:
    ...
    Example: GT_maxclique_easy_h256_L4_H8_nope_s2025
    """
...
class GTLayer(nn.Module):
    """
    Single Graph Transformer layer (TransformerConv) with pre-norm residuals.
    Edge-attribute aware: attention weights conditioned on edge embeddings.
    """
    def __init__(self, hidden_dim: int, num_heads: int,
                 dropout: float = 0.0, edge_dim: int = None):
        super().__init__()
...
class GTForNodeClassification(nn.Module):
    """
    Graph Transformer node classifier for max-clique.
...
def main():
    parser = argparse.ArgumentParser(description="GT — Max Clique Node Classification")
    ...
    # ── Model ───────────────────────────────────────────────────────────────
    model        = GTForNodeClassification(node_in_dim, edge_in_dim, config).to(device)
    ...
    row = {
        "model": "GT", "dataset": config["dataset_name"], "seed": config["seed"],
